...
| 工具 | 描述 |
|------|------|
| `team_create` | 创建新 Team（参数：name, description, project_path） |
| `team_delete` | 删除 Team |
| `team_status` | 获取 Team 状态（成员、任务、消息） |
| `inbox_read` | 读取成员 inbox |
| `inbox_send` | 发送消息到成员 inbox |
| `team_pending_requests` | 获取等待中的权限请求 |

### Team 编排工具（Agent Teams 专家）

| 工具 | 描述 |
|------|------|
| `team_orchestrate` | 根据任务描述自动创建团队并启动 Agents |
| `team_spawn_agent` | 在团队中启动新 Agent |
| `team_progress` | 获取团队聚合进度（成员状态、等待输入） |
| `team_shutdown` | 优雅关闭团队（停止所有 Agents） |
...
| 工具 | 描述 |
|------|------|
| `get_pending_confirmations` | 获取所有待处理的确认请求 |
| `reply_pending` | 回复待处理确认（支持 y/n/1/2/3 快捷回复） |
| `handle_user_reply` | 处理自然语言回复（自动解析意图） |
...
| 用户可能说的 | 意图 | 对应操作 |
|-------------|------|---------|
| "帮我修复 xxx bug" / "在 xxx 项目做 yyy" | 创建 Team 执行任务 | team_create + 自动启动 |
| "看看 team 在干嘛" / "team 进度怎样" | 查看 Team 状态 | team_status |
| "xxx team 怎么了" / "xxx 项目进度" | 查看特定 Team | team_status + team_name |
| "停掉 xxx team" / "取消 xxx 任务" | 删除 Team | team_delete |
| "y" / "n" / "允许" / "拒绝" | 回复权限请求 | inbox_send 到等待中的 agent |
| "看看消息" / "有什么通知" | 查看 inbox | inbox_read |
| "告诉 developer xxx" / "给 xxx 发消息" | 发送消息 | inbox_send |
| "有什么等着我" / "待处理" | 查看待处理请求 | team_pending_requests |
...
助手: 📊 当前 Team 状态
      ...
      myapp-login-fix
      ├── developer: 执行中 (context 45%)
      │   最近: 编辑 src/auth.rs
      └── 任务: 2/3 完成

用户: 有什么等着我
助手: 1 个待处理请求:
      🔐 myapp-login-fix/developer
      执行: npm test
      回复 y 允许，n 拒绝

用户: y
助手: 已发送，developer 继续了
```
