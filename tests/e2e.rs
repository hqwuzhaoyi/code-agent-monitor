...
use code_agent_monitor::mcp::{McpRequest, McpServer};
...
async fn test_e2e_multiple_agents() {
    ...
    call_method(
        &server,
        "agent/stop",
        serde_json::json!({ "agent_id": id1 }),
    )
    .await;
    call_method(
        &server,
        "agent/stop",
        serde_json::json!({ "agent_id": id2 }),
    )
    .await;
}
